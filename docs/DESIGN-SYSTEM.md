...
### The homepage is one argument, and the order is the design

Each section answers the question the previous one raises. `app/page.tsx`
carries the full map; the two placements that do the most work:

- **PastDrops sits directly after the calendar.** "Should I wait for a better
  one?" is the exact thought browsing produces, and an archive of sold-out
  releases is the answer to it. Anywhere else on the page it is trivia.
- **Scarcity sits last, immediately before the FAQ.** Urgency spent early is
  urgency wasted: a deadline only converts a reader who has already been given
  reasons to want the thing. It is the only section allowed to say *when*.

Certification (what the standard is) and Standard (the numbers behind it) run
together as one trust movement rather than being split by the saving.

**Every figure on the page is derived, never authored.** Savings computes its
headline percentage and every row from `getLiveDrop()`; Scarcity reads the same
adapter and shows the *scarcest* device rather than the first, because its job
is a deadline and 2-of-10 makes that case where 8-of-14 does not. A homepage
that restates the hero's numbers from a second source will eventually
contradict it, and a visible contradiction about price costs more trust than
the section was ever going to earn.

**Length is a design constraint here.** Twelve sections is 13.6 screens at
1440×900 — adding the four new ones cost 3,850px, and 430px of that was won
back by tightening the FAQ's row rhythm (set for six questions, now eleven) and
the certification rows. Anything added from here should replace, not append.

- **Hero** (`hero/`) — a limited release in progress, not a shop window (Apple keynote × Nothing × Rye Island). Full-viewport (`min-h-svh`), three quiet columns: drop label + statement + two CTAs left, the floating device centre, what is left of it right. **Monochrome by rule** — no copper, no italic, no badges, no fold bar, and no sale graphics; the section's only decoration is one enormous ghosted word (`REIMAGINED`, `text-ink/[0.03]` rising to `[0.06]` from `sm`, `text-[15vw]`, `tracking-[0.015em]`) sitting behind the product. **Fill the line with tracking, never with size.** Font-size grows the word on both axes and the vertical axis is spoken for — the live-drop panel sits to its right and the clearance beneath was tuned by hand. Measured at 1280×800, clearance is identical across `-0.045em`, `-0.02em`, `0` and `+0.02em` while ink runs 84 → 94%; `16.5vw` bought the same 92% fill and cost 20px of clearance. Each word has its own ratio — `REIMAGINED` measures 5.60× its font-size, `REFURBISHED` measured 6.20× — so a swap means re-tuning the tracking and leaving 15vw alone. The positive tracking is a deliberate exception to the tight display setting used elsewhere: at this size and 3–6% opacity the word reads as a masthead, not a headline.
  - **The word makes room for the rail with a mask, not a cut.** From `lg` up, `mask-image: linear-gradient(to right, black 70%, transparent 100%)` fades the tail into paper so the ink dissolves before it reaches the panel. A hard clip was tried and reverted — `REIMAGINE` reads as a typo; a soft taper reads as a masthead half-cropped by a page edge. Two earlier attempts sat here first: an opaque rail (which lost its own paper texture — see "re-lay the grain in any opaque panel" below) and pure vertical separation (which held at 1440×900 but the bottom-anchored panel rides up on shorter windows and put the baseline −13px into the panel at 1280×800). Measured with the mask: 108–118px of clean air between vanished ink and panel edge across 1024/1280/1440/1920, and the panel is free to grow without breaking it. Off below `lg` — no rail beside the word at those sizes.
  ...
  - **Currency** is `AED`/`en-AE` on the live drop — the company trades out of Dubai, so the storefront prices in dirhams. The stored amounts are AED price points in their own right, **not** figures converted from another currency at a rate: a conversion reads as an afterthought and lands on numbers no retailer would choose. `upcomingDrops` still carries USD in its (unrendered) data — align it when the catalogue is wired so one page can never print two currencies.
  - **A currency swap is a layout change.** `AED 1,999` is nine characters where `₹8,999` was six, at display scale. It cleared the price block (150px ink in a 335px phone column) but broke the drop-list: the selected row also carries a `Showing` label, and the longer string wrapped that one row to 90px against the others' 74px. Fixed by shortening its meta to `3 left` — the name above already supplies the noun. Re-measure the widest price string in both the price block and the list after any currency change.
...
### Conversion sections (added in the flow pass)

- **PastDrops** (`past-drops/`) — proof of demand. An *archive*, not a shelf: nothing is a link, nothing is buyable, images are `grayscale` (no opacity fade — the product has to stay identifiable), and the sold-out mark is a hairline strip rather than a stamp. That restraint is the argument — a red SOLD OUT badge reads as a shop that failed to restock, while a photograph under a thin label reads as a record of things that went fast. Same fact, opposite feeling, and only one of them makes the next drop urgent.
  - **Reading order matches the brief's hierarchy**: name → price → variant → struck original + `% off` (in `text-urgent`) → `N / N claimed` (fraction over sentence, "15 / 15" reads as scarcity in one glance) → drop + date on a hairline, last. Metadata that used `text-ink-faint` moved to `text-ink-muted` — the token tables reserve `-faint` for decorative-only, and calling a real fact "decorative" made it read as disabled.
  - **Price stays ink, not urgent.** The card is history: an orange price on a sold-out product would falsely signal "buy me". Ink at display scale (`1.375rem`) keeps it real without looking active. Discount is where the orange lives.
  - **Layout: rail below `md`, 2-column grid at `md`, 4-column at `xl`.** Mobile card is `w-[90%]` of the padded content area — the primary card fills the frame with ~10–12% of the next visible past the gutter (the brief's "≈1.1 cards visible" measured against the viewport, not the container). Tablet 2-up and desktop 4-up cells match to the pixel across the row so nothing looks squeezed by a longer product name.
- **Certification** (`certification/`) — the escape from a corporate icon grid is to stop drawing icons and start setting type. The numerals *are* the graphics: hairline-ruled rows, oversized index left, plain fact right — how a specification is printed, not how a feature is sold. The `INSPECTED → GRADED → WARRANTED → VERIFIED → READY` rail above gives the shape before the detail and adds no new information. Numbers stay ink: this section's job is credibility, and orange here would say "offer" where the page needs "standard".
- **Savings** (`savings/`) — reads product → price → saving, in that order of emphasis. Each card opens on the device itself in a **fixed 144/160px frame with `object-contain`** — not a fixed image size, because the cutouts are a tall phone, a wide laptop and a round watch, and only a shared frame makes three different silhouettes read as one set. The price is the largest thing after the photograph (`2.25rem`), the saving is stated in currency rather than percent (what stays in your pocket is the figure that lands), and the original is faint. Hover lifts the device 6px and scales it 3%, and the `You save` label warms muted → ink: the only thing in the card that gains weight when you reach for it. Closes on an editorial `View device →` linking to that device's own page, not a filled pill — a button in every cell would turn a value argument into the product grid it is deliberately not. One accent per row, on the saving alone. Prices ink, struck originals faint, no badge or burst anywhere. The headline `Up to N% less` is the largest **real** saving in the drop, computed at render, so it can never become a marketing ceiling that outlives the prices under it. Each row closes on the amount in currency, not percent — what stays in your pocket is the figure that lands.
- **Stories** (`stories/`) — pull-quotes on hairlines, not cards with avatars. The card-plus-avatar shape is what every SaaS page uses and it reads as marketing on sight; set as editorial quotes with the device beneath, the same words read as reporting. Scales to any number of reviews through `TestimonialCarousel`: a native scroll-snap track, so swipe momentum, keyboard scrolling and overflow-safety come from the platform rather than a slider library. **`perView` is measured** (track width ÷ card width), never a JS breakpoint list — 3 · 2 · 1 lives entirely in the class names, and paging clamps to the last full row so the final page is never ragged. The arrows only call `scrollTo`; scroll position remains the single source of truth, so the counter stays correct however the reader moves. Controls hide themselves when the collection already fits. No autoplay — the reader keeps control.
  **On the aggregate**: the `4.9/5 · 128 verified purchases`-style summary is *derived* by `getReviewSummary()`, never written down. A hardcoded rating is a factual claim about a business, not a layout placeholder, and it is the one value that survives the swap to real content without anyone noticing it was invented. Derived, it cannot: the figure reports the truth about whatever is in the adapter, and drops out entirely when no review carries a rating. Copy in `lib/testimonials.ts` is marked placeholder and must be replaced before launch.
- **Scarcity** (`scarcity/`) — the closing argument, as one horizontal band rather than a second hero. By this point the page has given product, proof, certification, saving, categories, process and social proof; every one was a reason to buy and none of them said *when*. Full-height here would read as the site starting over rather than closing.

### Drop card states (`upcoming-drops/drop-card.tsx`)

Four states drive label, accent, clock, stock line and action from one `STATES`
record rather than scattered conditionals — `available` (Shop now),
`coming-soon` (Notify me, countdown, no stock), `almost-gone` (Grab it, urgent
accent), `sold-out` (muted photograph, **no button at all**). A disabled
control invites a click that can never do anything, so sold-out gets plain type
instead. The calendar deliberately shows one closed drop: a grid where every
card looks the same teaches a shopper nothing, and seeing one already gone is
the cheapest possible proof that these run out.
